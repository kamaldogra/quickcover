/* global samanthamorelBgColors, samanthamorelPreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( samanthamorelBgColors ).forEach( function( context ) {
					samanthamorelGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function samanthamorelGenerateColorA11yPreviewStyles( context ) {
		// Get the accessible colors option.
		var a11yColors = window.parent.wp.customize( 'accent_accessible_colors' ).get(),
			stylesheedID = 'samanthamorel-customizer-styles-' + context,
			stylesheet = $( '#' + stylesheedID ),
			styles = '';
		// If the stylesheet doesn't exist, create it and append it to <head>.
		if ( ! stylesheet.length ) {
			$( '#samanthamorel-style-inline-css' ).after( '<style id="' + stylesheedID + '"></style>' );
			stylesheet = $( '#' + stylesheedID );
		}
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			// Check if we have elements defined.
			if ( samanthamorelPreviewEls[ context ] ) {
				_.each( samanthamorelPreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		samanthamorelGenerateColorA11yPreviewStyles( 'content' );
		samanthamorelGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
